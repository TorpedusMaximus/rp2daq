...
print(
    "Configuring PWM channel to make artificial 5kHz square wave (and waiting for it to settle)"
)
rp.pwm_configure_pair(
    0, clkdiv=250, wrap_value=200  # clock at 1000.0 kHz
)  # one rising edge, one falling edge at 5.000 kHz

time.sleep(0.08)  # unclear why, but 100 ms is safe
...
time.sleep(after_wait_sec)
print(
    f"Total received edge reports after another extra {after_wait_sec} seconds:", count
)
