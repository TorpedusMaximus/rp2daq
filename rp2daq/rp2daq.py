...
class Rp2daq:
    def __init__(self, required_device_id="", verbose=False):
        ...
        self._i = Rp2daq_internals(
            externals=self, required_device_id=required_device_id, verbose=verbose
        )
...
class Rp2daq_internals(threading.Thread):
    def __init__(self, externals, required_device_id="", verbose=False):
        ...
        self.port = self._find_device(
            required_device_id, required_firmware_version=MIN_FW_VER
        )
        ...
        self.data_receiving_thread = threading.Thread(
            target=self._data_receiver, daemon=True
        )
        self.report_processing_thread = threading.Thread(
            target=self._report_processor, daemon=True
        )
        self.callback_dispatching_thread = threading.Thread(
            target=self._callback_dispatcher, daemon=True
        )
    ...
    def _register_commands(self):
        ...
        (
            self.report_header_lenghts,
            self.report_header_formats,
            self.report_header_varnames,
        ...
        while self.run_event.is_set():
            try:
                if len(self.rx_bytes):
                    ...
                    report_args = struct.unpack(
                        self.report_header_formats[report_type],
                        bytes([report_type] + report_header_bytes),
                    )
                    cb_kwargs = dict(
                        zip(self.report_header_varnames[report_type], report_args)
                    )

                    data_bytes = []
                    dc, dbw = cb_kwargs.get("_data_count", 0), cb_kwargs.get(
                        "_data_bitwidth", 0
                    )
                    if dc and dbw:
                        # integer division is like floor(); this makes it ceil()
                        payload_length = -((-dc * dbw) // 8)
                        ...
                        if dbw == 8:
                            ...
                            dbw == 12
                        ):  # decompress 3B  into pairs of 12b values & flatten
                            odd = [
                                a + ((b & 0xF0) << 4)
                                for a, b in zip(data_bytes[::3], data_bytes[1::3])
                            ]
                            even = [
                                (c & 0xF0) // 16 + (b & 0x0F) * 16 + (c & 0x0F) * 256
                                for b, c in zip(data_bytes[1:-1:3], data_bytes[2::3])
                            ]
                            cb_kwargs["data"] = [x for l in zip(odd, even) for x in l]
                            if len(odd) > len(even):
                                cb_kwargs["data"].append(odd[-1])

                        elif dbw == 16:  # using little endian byte order everywhere
                            cb_kwargs["data"] = [
                                a + (b << 8)
                                for a, b in zip(data_bytes[:-1:2], data_bytes[1::2])
                            ]

                        # if max(cb_kwargs["data"]) > 2250: print(data_bytes) # XXX

                    # false for unexpected reports
                    ...
                    if cb:
                        ...
                        # unblock default callback (& send it data)
                        self.sync_report_cb_queues[report_type].put(cb_kwargs)
                    elif (
                        cb is False
                    ):  # unexpected report, from command that was not yet called in this script instance
    ...
    def default_blocking_callback(self, command_code):
        ...
        # waits until default callback unblocked
        kwargs = self.sync_report_cb_queues[command_code].get()
        return kwargs
    ...
    def _find_device(self, required_device_id, required_firmware_version=0):
        ...
        for port_name in port_list:
            ...
            if not port_name.hwid.startswith(
                "USB VID:PID=2E8A:000A SER=" + required_device_id.upper()
            ):
                continue
            ...
            try:
                ...
                time.sleep(0.15)  # 50ms round-trip time is enough
            ...
            if not id_data[:6] == b"rp2daq":
                logging.info(
                    f"\tport open, but device does not identify itself as rp2daq: {id_data}"
                )
                continue

            version_signature = id_data[7:13]
            if (
                not version_signature.isdigit()
                or int(version_signature) < required_firmware_version
            ):
                logging.warning(
                    f"rp2daq device firmware has version {version_signature.decode('utf-8')},\n"
            ...
            if required_device_id and found_device_id != required_device_id:
                logging.info(
                    f"found an rp2daq device, but its ID {found_device_id} does not match "
                    + f"required {required_device_id}"
                )
                continue

            logging.info(
                f"connected to rp2daq device with manufacturer ID = {found_device_id.decode()}"
            )
            return try_port
        else:
...
if __name__ == "__main__":
    print(
        "Note: Running this module as a standalone script will only try to connect to a RP2 device."
    )
