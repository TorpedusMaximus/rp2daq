...
channels = [
    ...
    1,
    2,
]  # ,  3, 4]     # 0,1,2 are pins 26-28;  3 is V_ref and 4 is internal thermometer
...
class StupidPlot(tkinter.Frame):
    ...
    def plot(
        self, channel_data
    ):  # That's it. 3 lines of code to plot data as coloured lines.
        for values, color in zip(
            channel_data, ("red2", "yellow2", "green2", "blue2", "violet")
        ):
            self.drawnlines.append(
                self.canvas.create_line(*enumerate(values), fill=color)
            )
