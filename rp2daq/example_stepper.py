...
zero_pos = {}
zero_pos[0] = rp.stepper_init(0, dir_pin=12, step_pin=13, endswitch_pin=19, inertia=90)[
    "initial_nanopos"
]
zero_pos[1] = rp.stepper_init(1, dir_pin=10, step_pin=11, endswitch_pin=18, inertia=90)[
    "initial_nanopos"
]
# print('\n'*3); time.sleep(.05)
...
def stepper_cb(**kwargs):
    ...
    if kwargs["endswitch_triggered"]:
        if kwargs["endswitch_expected"]:
            zero_pos[kwargs["stepper_number"]] = kwargs[
                "nanopos"
            ]  # recalibrate position against endstop
        else:
            print(
                "Error: Unexpected endswitch trigger on motor #{kwargs['stepper_number']}!"
            )
            coords_to_go[
                :
            ] = []  # for safety, we flush further movements, so this script ends
...
for st in (0, 1):
    rp.stepper_move(
        st, to=zero_pos[0] - 10000000, speed=128 * 2, _callback=stepper_cb
    )  # moving towards 0 = seeking end switch
    print("\n" * 3)
    time.sleep(0.05)
